import { AnimateOnScroll, ScrollTextReveal, StaggerChildren } from "@/components/ui";
import { PRIMARY_CTA_HREF } from "@/content/site";

/* eslint-disable @next/next/no-img-element */

export function AlternateCTASection() {
  // return (
  ...
  //             <div
  //               key={point}
  //               className="relative w-40 md:w-auto h-10 md:h-12 rounded-[40px] md:rounded-[50px] overflow-hidden outline outline-[0.80px] outline-offset-[-0.80px] outline-white/30 md:outline-none backdrop-blur-xl md:backdrop-blur-[30px] inline-flex justify-center items-center"
  //             >
  //               <span aria-hidden="true" className="hidden md:block absolute inset-0 rounded-[50px] bg-gradient-to-b from-white/30 to-white/[0.04]" />
  //               <span aria-hidden="true" className="hidden md:block absolute inset-[1px] rounded-[50px] bg-[#131313]" />
  //               <div className="relative z-10 inline-flex justify-start items-center gap-4 md:gap-5 pl-7 pr-6 md:pl-9 md:pr-7 py-6 md:py-7">
  ...
  return (
    <section className="relative bg-black overflow-hidden w-[100vw] left-1/2 -translate-x-1/2 px-4 py-10 md:px-8 md:py-20" id="cta">
      ...
      <img
        ...
        alt=""
        className="pointer-events-none absolute left-[45%] top-[30%] -translate-x-1/2 -translate-y-1/2 w-full max-w-[1400px] h-auto opacity-100"
      />
      {/* Bright stars — wave highlight sweep */}
      <img
        src="/images/figma/ctasection.svg"
        alt=""
        className="stars-wave-bright pointer-events-none absolute left-[45%] top-[30%] -translate-x-1/2 -translate-y-1/2 w-full max-w-[1400px] h-auto"
      />
      {/* Top gradient glow */}
