...
function CheckBullet({ text }: { text: string }) {
  return (
    <div className="self-stretch flex items-center gap-[12px]">
      ...
      <span className="text-gray-300 text-[16px] font-medium leading-[36px]">{text}</span>
    </div>
  );
}
...
function ListCard({ title, items }: { title: string; items: string[] }) {
  return (
    ...
    >
      <h3 className="self-stretch text-white text-[20px] font-bold leading-[24px]">{title}</h3>
      <div className="self-stretch flex flex-col gap-[15px]">
        {items.map((item) => <CheckBullet key={item} text={item} />)}
      </div>
    </div>
  );
}
...
function ProfileCard({ name, role, avatar }: { name: string; role: string; avatar: string }) {
  return (
    <div
      ...
      style={{
        paddingTop: 18, paddingBottom: 18, paddingLeft: 16, paddingRight: 16,
        background: "#121212",
    ...
    >
      {/* eslint-disable-next-line @next/next/no-img-element */}
      <img src={avatar} alt={name} className="w-[56px] h-[56px] rounded-full shrink-0 object-cover" />
      <div className="flex flex-col gap-[6px] flex-1">
        <div className="flex items-center gap-[10px]">
          <span className="text-white text-[17px] font-bold leading-[28px]">{name}</span>
          <div className="w-[7px] h-[7px] rounded-full bg-green" />
          <span className="text-gray-100 text-[14px] font-bold leading-[22px]">Active</span>
        </div>
        <div className="h-[32px] px-[14px] py-[6px] bg-white/6 rounded-[20px] inline-flex items-center w-fit">
          <span className="text-white text-[14px] font-medium leading-[20px]">{role}</span>
        </div>
      </div>
      <DotsMenu />
    </div>
  ...
  return (
    <div className="relative w-[520px] h-[530px] shrink-0">
      ...
      <div
        className="absolute"
        style={{
          left: 50, top: 55, width: 450, height: 420,
          background: "#111111",
          boxShadow: "0px 0px 7.65px rgba(255,255,255,0.27) inset, 0px 4.05px 4.05px rgba(0,0,0,0.30)",
          borderRadius: 50,
        }}
      >
      ...
      <div className="absolute" style={{ right: 0, top: 185 }}>
        <ProfileCard name="Steve Jan" role="UX Designer" avatar="/images/figma/4229-62012/imgEllipse55.png" />
      </div>
